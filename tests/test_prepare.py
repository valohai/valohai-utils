...
def test_prepare(tmpdir, monkeypatch):
    ...
    assert (
        get_input_info("myimages", download=DownloadType.NEVER).files[0].uri
        == "https://upload.wikimedia.org/wikipedia/commons/8/84/Example.svg"
    )
    assert (
        get_input_info("myimages", download=DownloadType.NEVER).files[1].uri
        == "https://upload.wikimedia.org/wikipedia/commons/0/01/Example_Wikipedia_sandbox_move_UI.png"
    )
    assert not get_input_info("overrideme", download=DownloadType.NEVER).files[0].uri
    assert os.path.isfile(
        get_input_info("overrideme", download=DownloadType.NEVER).files[0].path
    )
