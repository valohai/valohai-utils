...
def get_inputs_path(input_name: Optional[str] = None) -> str:
    if is_running_in_valohai():
        ...
        path = os.environ.get(
            "VH_INPUTS_DIR", os.path.join(VH_LOCAL_INPUTS_DIR, global_state.step_name or "default")
        )
...
def get_outputs_path() -> str:
    if is_running_in_valohai():
        ...
        return os.environ.get(
            "VH_OUTPUTS_DIR",
            os.path.join(
                VH_LOCAL_OUTPUTS_DIR, get_execution_guid(), global_state.step_name or "default"
            ),
        )
